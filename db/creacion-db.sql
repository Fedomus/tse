...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-03-2023 a las 20:07:15
...
('L1mbmnx_XRDbc37RPAodTkrW5fiYxajB', 1678388716, '{\"cookie\":{\"originalMaxAge\":null,\"expires\":null,\"httpOnly\":true,\"path\":\"/\"},\"usuario\":\"prueba\",\"tipo\":2,\"area\":6,\"usuarioLogueado\":true}');
...
CREATE TABLE `tags` (
  ...
  `tagsnombre` varchar(255) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(5, 'Museos de Película'),
(6, 'Latinoamérica'),
(7, 'Cine.ar TV'),
(8, 'Gafas Violetas'),
(9, 'Análisis de Datos'),
(10, 'Estreno Nacional'),
(11, 'Festivales Nacionales'),
(12, 'Cine en Cárceles'),
(13, 'Gaumont'),
(14, 'Las Escuelas Van al Cine'),
(15, 'Transversalidad'),
(16, 'Festival Internacional de Mar del Plata'),
(17, 'Espacios INCAA'),
(18, 'Integración Regional'),
(19, 'Cine Independiente'),
(20, 'Catálogo'),
(21, 'Digitalización'),
(22, 'Promoción'),
(23, 'Acervo');
...
CREATE TABLE `tarjetas` (
  ...
  `tarjetastags` varchar(255) DEFAULT NULL,
  ...
  `tarjetasultmofusuario` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
INSERT INTO `tarjetas` (`idtarjetas`, `tarjetastitulo`, `tarjetascuerpo`, `tarjetasexpediente`, `tarjetasactoadministrativo`, `tarjetastags`, `tarjetasautor`, `tarjetasareas`, `tarjetasejes`, `tarjetasobjetivos`, `tarjetaslineas`, `tarjetasplandeaccion`, `tarjetasestado`, `tarjetasfecha`, `tarjetasultmodfecha`, `tarjetasultmofusuario`) VALUES
(1, 'Catálogo Audiovisual Argentino', 'Se ha puesto on line luego de finalizadas las etapas de armado de la base de datos, diseño y maquetado. Se han incluído largometrajes y cortometrajes 2021 y 2022. Se inicia la etapa de recopilación de datos de largos y cortos cuya realización sea previa a 2022 mediante mailing a productores y distribuidores a través de INCAA en línea.', '', '', 'Catálogo, Digitalización, Promoción, Acervo', 1, 1, 2, 5, 15, 60, 1, '2023-02-16 18:52:15', '2023-03-07 22:40:18', 1);
...
(1, 'prueba', '$2b$10$m4a4gq7E27lYO1NQHpdL0ubtJAxH.i77cJraqhBN73WL.CZzZtgdy', 6, 2);
...
SQL SECURITY DEFINER VIEW `pegi`  
AS SELECT `p`.`idplanesdeaccion` 
AS `idplanesdeaccion`, `p`.`planesdeaccionnombre` 
AS `planesdeaccionnombre`, `p`.`planesdeaccionlinea` 
AS `planesdeaccionlinea`, `p`.`planesdeaccionarea` 
AS `planesdeaccionarea`, `a`.`idareas` 
AS `idareas`, `a`.`areasnombre` 
AS `areasnombre`, `l`.`idlineas` 
AS `idlineas`, `l`.`lineasnombre` 
AS `lineasnombre`, `l`.`lineasobjetivo` 
AS `lineasobjetivo`, `o`.`idobjetivos` 
AS `idobjetivos`, `o`.`objetivosnombre` 
AS `objetivosnombre`, `o`.`objetivoseje` 
AS `objetivoseje`, `e`.`idejes` 
AS `idejes`, `e`.`ejesnombre` 
AS `ejesnombre` 
FROM ((((`planesdeaccion` `p` left join `areas` `a` 
on(`p`.`planesdeaccionarea` = `a`.`idareas`)) 
left join `lineas` `l` on(`p`.`planesdeaccionlinea` = `l`.`idlineas`)) 
left join `objetivos` `o` on(`l`.`lineasobjetivo` = `o`.`idobjetivos`)) 
left join `ejes` `e` on(`o`.`objetivoseje` = `e`.`idejes`))  ;
...
ALTER TABLE `tarjetas`
  ...
  ADD KEY `tarjetas_ultmodusuarios` (`tarjetasultmofusuario`);
...
--
ALTER TABLE `tags`
  MODIFY `idtags` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=24;

--
-- AUTO_INCREMENT de la tabla `tarjetas`
--
ALTER TABLE `tarjetas`
  MODIFY `idtarjetas` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=54;
...
ALTER TABLE `tarjetas`
  ...
  ADD CONSTRAINT `tarjetas_ultmodusuarios` FOREIGN KEY (`tarjetasultmofusuario`) REFERENCES `usuarios` (`idusuarios`),
